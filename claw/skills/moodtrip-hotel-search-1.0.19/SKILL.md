...
On OpenClaw, MCP tools may be exposed with the server-name prefix. Treat any of
these names as available MoodTrip tools too: `moodtrip__searchHotelsWithRates`,
`moodtrip__findHotels`, `moodtrip__search_hotels_by_description`,
`moodtrip__build_booking_link`, `moodtrip__getHotelDetails`.

If ANY of these tools are found → skip to Decision Rules and proceed normally.

**Runtime detection:** If you are running on OpenClaw, Codex, or any OpenAI-compatible runtime that validates tool schemas strictly, **prefer `search_hotels` and `get_hotel_details`** over `searchHotelsWithRates` and `getHotelDetails` when the unprefixed wrapper tools exist. If OpenClaw exposes only prefixed tools, use `moodtrip__searchHotelsWithRates` for priced hotel search and `moodtrip__getHotelDetails` for hotel details instead of marking MoodTrip unavailable.
...
- OpenClaw currently supports **stdio MCP servers only** (not HTTP). Use the `mcp-remote` bridge:
  - Add to config: `mcp.servers.moodtrip = { "command": "npx", "args": ["-y", "mcp-remote@latest", "https://api.moodtrip.ai/api/mcp-http"] }`
...
On OpenClaw, the same tools can appear as `moodtrip__searchHotelsWithRates`,
`moodtrip__getHotelDetails`, and `moodtrip__build_booking_link`. Use those
prefixed names when they are the names exposed to the agent.
